...
with open('pi_digits.txt') as file_object: # | with所求值的对象必须有一个enter()方法，一个exit()方法。
                                           # | 紧跟with后面的语句被求值后，返回对象的enter()方法被调用，这个方法的返回值赋值到as后面的变量
                                           # | 当with后面的代码块全部被执行完，将调用前面返回对象的exit()方法



    contents = file_object.read() # 使用方法read()读取这个文件全部内容，将其作为一个字符串存储到变量contents中
